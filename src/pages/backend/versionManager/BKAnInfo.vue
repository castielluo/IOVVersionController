<template>
    <div class="box-an-info">
      <h1>Android
        <router-link to="/bristlegrass/anadd"><Button type="info" class="add-btn">添加</Button></router-link>
      </h1>
      <Row>
        <Col span="24">
          <Card v-for="(item, index) in dataInfo" v-bind:key="item.id">
            ...
            <ul class="version-ul">
              ...
              <li>
                ...
                <div class="version_content">Android</div>
              </li>
              <li>
...
export default {
  ...
  mounted () {
    let self = this
    this.get('/iov_apis/android/versions', {token: this.token}).then(function (res) {
